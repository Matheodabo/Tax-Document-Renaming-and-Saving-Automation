...
def extract_pdf_fields(pdf_path: Path, debug: bool = False) -> dict:
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            words = page.extract_words(x_tolerance=6, y_tolerance=3)
            ...
            #
            # Method 1 (primary): crop top-left region → extract_text()
            #   extract_text() reconstructs words from characters, avoiding the
            #   pdfplumber word-split bug where "SAPERSTEIN" becomes "SAPER"+"STEIN".
            #   After payer address block (lines with digits), the first non-digit
